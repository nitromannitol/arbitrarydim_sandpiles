include("sandpile_cpu_multidim_init.jl")

#############
## sample 1
## -------
## stabilize the all 7s sandpile in 3D 
## on a hypercube of side length 2^4
d = 3
M= 2^4
init_condition = 7
...
## with ~2^14 number of chips at the center
d = 3
M= 2^4
N = getHyperTetrahedral(d,M);
init_condition = fill(0,N)
init_condition[1] = d*N 
#takes about 5 seconds on 2013 macbook pro
@time v, index, num_iters = getOdometer_cpu(init_condition,M,d) 
s = getLaplacian_cpu(v,index,M,d)
s = s + init_condition
S = fillOut(s,index,M,3)
T = fillOut(v,index,M,3)
...
## of 2d + k for  d = 2 ... 6
## and k = 0 ... 4
## this produces a low resolution version of 
## Table 1 in "Dynamic Dimensional Reduction ..."
...
# set the hypercube radius
kk = 3
M = 2^kk

for d in 2:6
	for k in 0:4
		println("computing dimension $d, k: $k")
		slice_dimension = 2
		init_condition = 2*d+k
		v,index,n = getOdometer_cpu(init_condition,M,d)
		s = getLaplacian_cpu(v,index,M,d)
